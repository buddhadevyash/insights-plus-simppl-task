    ...
    useEffect(() => {
        ...
        const fetchDataAndBuildGraph = async () => {
            ...
            try {
                ...
                const allClusterNodes = result.clusters.map(c => ({ id: c.cluster_id, label: `${c.cluster_name}\n(${c.item_count})`, group: 'cluster', value: c.item_count, fullData: c }));
                const allUserNodes = [];
                const allEdges = [];
                const userNodeIds = new Set();
                const clusterNodeIds = new Set(result.clusters.map(c => c.cluster_id));

                result.data.forEach(item => {
                    const postNodeId = `${item.username}_${item.id}`;
                    if (item.username && !userNodeIds.has(postNodeId)) {
                        allUserNodes.push({ id: postNodeId, label: item.username, group: 'user', value: 1, fullData: item });
                        userNodeIds.add(postNodeId);
                    }
                    if (clusterNodeIds.has(item.cluster_id)) {
                        allEdges.push({ id: `${postNodeId}->${item.cluster_id}`, from: postNodeId, to: item.cluster_id });
                    }
                });
